...
# Atomic file writes
# ----------------------------------------------------------------------
# Both files this script generates are read by other processes while it runs,
# so neither may ever be observed half-written.  supervisord does not order
# this oneshot against the long-running programs, so they can be spawned while
# this script is mid-write; source-env.sh handles that by waiting for the env
# file to appear, which is only sound if the file appears complete.  A plain
# `cat > file` creates the file empty and fills it afterwards, so a child can
# source an empty file and silently keep the image's baked-in placeholders
# (LILA_URL=http://localhost:8080), leaving lila serving 301s to localhost and
# lila-ws rejecting every handshake.  The auth_proxy has the same exposure on
# the credentials file, which it re-reads on every auto-login attempt.
#
# Writing to a temp file and renaming closes the window: rename(2) is atomic
# within a directory, so a reader sees either no file or the whole file.
# mktemp creates at 0600, so the content is never world-readable even before
# the explicit chmod. The EXIT trap matters because this script runs under
# `set -e`: without it, a failure between mktemp and mv would leave an
# orphaned temp file containing the admin password in cleartext.
_openhost_tmp_files=()
_openhost_cleanup_tmp() {
    local f
    for f in "${_openhost_tmp_files[@]+"${_openhost_tmp_files[@]}"}"; do
        rm -f "$f"
    done
}
trap _openhost_cleanup_tmp EXIT

# atomic_write <dest> <mode>; content is read from stdin.
atomic_write() {
    local dest="$1" mode="$2" tmp
    tmp="$(mktemp "${dest}.XXXXXX")"
    _openhost_tmp_files+=("$tmp")
    cat > "$tmp"
    chmod "$mode" "$tmp"
    mv -f "$tmp" "$dest"
}
...
atomic_write "$CRED_FILE" 0600 <<EOF
...
# ----------------------------------------------------------------------
# Mode 0600: this carries the admin password in cleartext, and every
# supervisord child sources it.  Written atomically; see atomic_write above
# for why that is required rather than merely tidy.
