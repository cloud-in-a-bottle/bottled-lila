...
umask 077
# Written atomically (write to a temp file, then rename) because the
# auth_proxy re-reads this file on every auto-login attempt and must never
# observe a half-written one.  rename(2) within a directory is atomic, so
# readers see either the old file or the complete new one.
CRED_TMP="$(mktemp "${CRED_FILE}.XXXXXX")"
cat > "$CRED_TMP" <<EOF
...
# ----------------------------------------------------------------------
# This MUST be atomic.  supervisord gives no ordering guarantee between this
# oneshot and the long-running programs, so they can be spawned while this
# script is still running; source-env.sh covers that by waiting for the file
# to appear.  But `cat > file` creates the file EMPTY and fills it after, so
# waiting only for existence leaves a window where a child sources an empty
# file, silently keeps the image's baked-in placeholders
# (LILA_URL=http://localhost:8080), and comes up misconfigured.  Observed on
# a real restart: lila-ws won the race and got the right origin while lila
# lost it and booted with localhost:8080, which made the whole site answer
# 301 to http://localhost:8080/ and left every socket handshake mismatched.
#
# Writing to a temp file and renaming closes the window: rename(2) is atomic
# within a directory, so a child either does not see the file yet (and keeps
# waiting) or sees it complete.  chmod before the rename so it is never
# briefly world-readable; it carries the admin password in cleartext and
# every supervisord child sources it.
